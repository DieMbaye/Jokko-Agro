...
\textbf{XSS --- Injection JavaScript} & C2 (IndexedDB non extractable) + C1 (signature requise) + C3 (vérif. serveur) \\
\rowcolor{bleuTresClair}
\textbf{Replay attack} & C4 (nonce UUID + timestamp) + C3 (vérif. Firestore used\_nonces) \\
\textbf{Manipulation image IA} & C5 (hash SHA-256 IPFS) + C3 (recalcul serveur indépendant) \\
\rowcolor{bleuTresClair}
\textbf{Double-dépense AGC} & C8 (escrow atomique) + C6 (ledger chaîné) + C9 (audit trail) \\
\textbf{Accès API directe (bypass Angular)} & C3 (Cloud Functions) + C10 (Firestore Rules) + C1 (ECDSA obligatoire) \\
\rowcolor{bleuTresClair}
\textbf{Falsification de transaction} & C6 (ledger SHA-256 chaîné) + C1 (signature) + C9 (audit) + C3 (vérif. serveur) \\
\textbf{Effacement de preuves} & C9 (write-only Firestore Rules: allow delete: if false) \\
\rowcolor{bleuTresClair}
\textbf{Vol de clé Firestore} & C7 (AES-GCM chiffrement) + C2 (clé privée jamais stockée en clair) \\
\end{tabularx}
\end{tcolorbox}
...
a été développé. Il exécute automatiquement 25 tests couvrant l'ensemble des mécanismes
...
\noindent
Les 25 tests sont organisés en 5 catégories correspondant aux couches de sécurité:
...
\caption{Résultats des tests de sécurité sur le Chiffrement AEES}
...
\vspace{0.3cm}
\renewcommand{\arraystretch}{1.5}
\begin{tcolorbox}[tablebox,title={Synthèse des tests de sécurité}]
\begin{tabularx}{\textwidth}{
  >{\bfseries\small\color{bleuUniv}}p{4cm}
  >{\small\bfseries}p{2cm}
  >{\small\bfseries}p{2cm}
  >{\small\bfseries}p{2cm}
  >{\small}X}
\rowcolor{bleuUniv}
\multicolumn{1}{>{\bfseries\color{white}\small}p{4cm}}{Catégorie} &
\multicolumn{1}{>{\bfseries\color{white}\small}p{2cm}}{Tests} &
\multicolumn{1}{>{\bfseries\color{white}\small}p{2cm}}{Pass} &
\multicolumn{1}{>{\bfseries\color{white}\small}p{2cm}}{Fail} &
\multicolumn{1}{>{\bfseries\color{white}\small}X}{Statut} \\
Signatures ECDSA (C1) & 5 & 5 & 0 & \textcolor{green}{\checkmark Validé} \\
\rowcolor{bleuTresClair}
Stockage IndexedDB (C2) & 3 & 3 & 0 & \textcolor{green}{\checkmark Validé} \\
Ledger immuable (C6) & 4 & 4 & 0 & \textcolor{green}{\checkmark Validé} \\
\rowcolor{bleuTresClair}
Séquestre AGC (C8) & 5 & 5 & 0 & \textcolor{green}{\checkmark Validé} \\
Chiffrement AES-GCM (C7) & 4 & 4 & 0 & \textcolor{green}{\checkmark Validé} \\
\rowcolor{bleuTresClair}
Audit Trail (C9) & 4 & 4 & 0 & \textcolor{green}{\checkmark Validé} \\
\hline
\textbf{Total} & \textbf{25} & \textbf{25} & \textbf{0} & \textbf{100\% succès} \\
\end{tabularx}
\end{tcolorbox}
